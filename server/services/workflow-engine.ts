...
export async function dispatchWorkflowEvent(payload: WorkflowEventPayload): Promise<void> {
    setImmediate(async () => {
        try {
            ...
            console.error("[WorkflowEngine] Unhandled error in dispatchWorkflowEvent:", err);
        }
    });
}
    ...
    for (let i = startIndex; i < actions.length; i++) {
        ...
        try {
            ...
            if (action.type === "wait") {
                ...
                console.log(`[WorkflowEngine] Workflow #${workflow.id} suspended. Resumes at ${resumeAt.toISOString()}`);
...
async function _executeAction(action: WorkflowAction, payload: WorkflowEventPayload, db: any): Promise<void> {
    switch (action.type) {
        ...
        default: console.warn(`[WorkflowEngine] Unknown action type: ${(action as any).type}`);
    }
}
...
async function _actionSendTemplate(action: SendTemplateAction, payload: WorkflowEventPayload, db: any): Promise<void> {
    ...
    while (attempts < maxAttempts) {
        try {
            ...
            console.warn(`[WorkflowEngine] HTTP 429 / Error sending template. Retrying in ${backoffDelay}ms...`);
