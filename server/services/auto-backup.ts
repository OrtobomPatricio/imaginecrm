...
export function startAutoBackup() {
    ...
    if (!enabled) {
        console.log("[AutoBackup] Disabled (set BACKUPS_ENABLED=1 to activate)");
        return;
    }

    const encryptionKey = process.env.BACKUP_ENCRYPTION_KEY;
    if (!encryptionKey || encryptionKey.length < 16) {
        console.error("[AutoBackup] REJECTED: BACKUP_ENCRYPTION_KEY is required and must be ≥16 chars. Backups will NOT run.");
        return;
    }

    console.log("[AutoBackup] Starting daily encrypted backup scheduler...");
    ...
    cron.schedule("0 2 * * *", async () => {
        try {
            ...
            if (!db) {
                console.error("[AutoBackup] Database not available, skipping.");
                return;
            }
            ...
            for (const tenantId of tenantIds) {
                try {
                    ...
                    console.log(`[AutoBackup] Created encrypted backup: ${backupPath} (tenant ${tenantId})`);
                } catch (tenantErr) {
                    console.error(`[AutoBackup] Error backing up tenant ${tenantId}:`, tenantErr);
                }
            }
            ...
            if (files.length > 7 * tenantIds.length) {
                files.slice(7 * tenantIds.length).forEach(f => {
                    try {
                        ...
                        console.error(`[AutoBackup] Error deleting old backup ${f}:`, e);
                    }
                });
            }
        } catch (err) {
