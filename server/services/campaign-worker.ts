...
async function processScheduledCampaigns() {
    ...
    for (const campaign of dueCampaigns) {
        ...
        try {
            ...
            if (acquired !== 1) {
                console.log(`[CampaignWorker] Campaign ${campaign.id} already being processed by another instance, skipping.`);
                continue;
            }
        } catch {
            // If advisory lock not available (e.g. MockDB), proceed but log warning
            console.warn(`[CampaignWorker] Advisory lock not available for campaign ${campaign.id}, proceeding without lock.`);
        }

        console.log(`[CampaignWorker] Starting campaign ${campaign.id}: ${campaign.name} (tenant ${campaign.tenantId})`);
        await db
            .update(campaigns)
...
async function processWhatsAppCampaignBatch(campaign: typeof campaigns.$inferSelect) {
    ...
    if (connections.length === 0) {
        console.warn(`[CampaignWorker] No active Meta Cloud API connections found for tenant ${campaign.tenantId}. Pausing campaign ${campaign.id}.`);
