...
export function startWorkflowPoller() {
    ...
    console.log(`[WorkflowPoller] Empieza el monitoreo de colas (Intervalo: ${POLL_INTERVAL_MS}ms)`);
    ...
    pollerInterval = setInterval(async () => {
        ...
        try {
            ...
            console.error("[WorkflowPoller] Error crítico en el ciclo:", err);
        } finally {
            isPolling = false;
        }
    }, POLL_INTERVAL_MS);
...
export function stopWorkflowPoller() {
    if (pollerInterval) {
        ...
        pollerInterval = null;
        console.log("[WorkflowPoller] Monitoreo detenido");
    }
}
...
async function pollPendingJobs() {
    ...
    console.log(`[WorkflowPoller] ⚡ Despertando ${readyJobs.length} jobs suspendidos...`);
    ...
    for (const job of readyJobs) {
        try {
            ...
            console.log(`[WorkflowPoller] Reanudando Job #${job.id} (Workflow #${job.workflowId}) desde acción #${job.actionIndex}`);
            ...
            console.error(`[WorkflowPoller] Error al reanudar Job #${job.id}:`, err);
            // Actualizar a failed
            await db.update(workflowJobs)
